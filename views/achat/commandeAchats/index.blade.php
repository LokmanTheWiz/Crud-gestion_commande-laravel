...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            ...
            <h1>
                listes des Commande Achats
            </h1>
            @if (session('success'))
            <div id="suc" class="alert-warning" role="alert">
                {{ session('success') }}
            </div>
            @endif
            ...
            @endif
            <div class="float-end">
                <a href="{{ route('commandeachat.create') }}">New commande</a>
            </div>
        <table class="table ">
            <thead>
                <tr>
                ...
                @foreach ($commandeAchats as $commandeAchat)
                    ...
                    <tr>               
                        ...
                        <form action="{{ route('commandeachat.destroy', $commandeAchat->id) }}" method="POST">                                    
                            {{ csrf_field() }}
                            {{method_field('DELETE')}}
                            <a href="{{ route('commandeachat.show', $commandeAchat->id) }}" class="btn btn-primary">
                            ...
                            <a href="{{ route('commandeachat.edit', $commandeAchat->id) }}" class="btn btn-warning">
        ...
        <div class="d-flex">{{ $commandeAchats->links() }}</div>
    </div>
@endsection
