    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                ...
                @endif
                <div class="float-end">
                    <a href="{{ route('fournisseur.create') }}">New Fournisseur</a>
                </div>
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <th> {{$fourni ->nom}}</th>
                        <th> {{$fourni ->telephone}}</th>
                        <th> {{$fourni ->email}}</th>
                        <th> {{$fourni ->ville}}</th>
                        <th> {{$fourni ->adresse}}</th>
                        <td>
                            <form action="{{ route('fournisseur.destroy', $fourni->id) }}" method="POST">
                                    
                                {{ csrf_field() }}
                                {{method_field('DELETE')}}
                                <a href="{{ route('fournisseur.show', $fourni->id) }}" class="btn btn-primary">
                                ...
                                <a href="{{ route('fournisseur.edit', $fourni->id) }}" class="btn btn-warning">
